...
const float GR = (sqrt(5) - 1) / 2;//黄金分割比 Golden Ratio
...
//	辅助函数:
	int H(KeyType row, KeyType col) const;					// 散列函数
	int Collision(KeyType row, KeyType col, int i) const;	// 处理冲突的函数
...
//  二叉树方法声明及重载编译系统默认方法声明:
	...
	int Search(const ElemType e) const ;	    // 查询e是否在表中
...
// 散列表类的实现部分
template <class ElemType, class KeyType>
int ClosedHashTable<ElemType, KeyType>::H(KeyType row, KeyType col) const
//操作结果: 返回散列地址
{
	return int(row * A + col * (1 - A)) % maxSize;
}

template <class ElemType, class KeyType>
int ClosedHashTable<ElemType, KeyType>::Collision(KeyType row, KeyType col, int i) const
//操作结果: 返回第i次冲突的探查地址
{
	return (H(row, col) + i) % maxSize;
}
...
template <class ElemType, class KeyType>
int ClosedHashTable<ElemType, KeyType>::Search(const ElemType e) const
// 操作结果: 查寻关键字为key的元素的值,如果查找成功,返回元素在数组的下标,否则返回-1. 
{
	int c = 0;							// 冲突次数
	int i = H(e.row, e.col);						// 元素的哈希函数值 

	while (c < maxSize && ((tag[i] == USE && ht[i] != e) || tag[i] == DELETE))
		// 冲突次数应小于maxSize且元素ht[pos]非空且关键字值不等
		i = Collision(e.row, e.col, ++c);		// 求得下一个探查地址
