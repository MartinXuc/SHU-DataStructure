...
//	辅助函数:
	...
	int LNodeCountHelp(const BinAVLTreeNode<ElemType>* r) const;			// 求以r为根的平衡二叉树的左子树节点个数
...
//  平衡二叉树方法声明及重载编译系统默认方法声明:
	...
	int LNodeCount() const;										// 求平衡二叉树的左子树结点个数
...
template <class ElemType>
int BinaryAVLTree<ElemType>::LNodeCount() const
{
	return LNodeCountHelp(root);
}
template <class ElemType>
int BinaryAVLTree<ElemType>::LNodeCountHelp(const BinAVLTreeNode<ElemType> *r) const
//求当前结点左子树的结点个数
{
	if (r == NULL)return -1;//将空指针传进来必然是出错了，空指针没有左子树
...
template<class ElemType>
void BinaryAVLTree<ElemType>::UpdateLsize()
{
	BinAVLTreeNode<ElemType>* p = GetRoot();
	UpdateLsize(p);
}
