...
// 并查集的函数成员:
	...
	void HeightUnion(ElemType a, ElemType b);
...
// 操作结果：合并a与b所在的等价类
{
	...
	if (r1 != r2 && r1 != -1) {
	   ...
	   sets[r1].height = max(sets[r1].height, sets[r2].height + 1);
    }
}
...
// 操作结果：合并a与b所在的等价类
{
	...
	if (r1 != r2 && r1 != -1) {
		...
		else
		{
			...
			sets[r1].parent = r2;	        // 合并等价类
			sets[r2].height = max(sets[r2].height, sets[r1].height + 1);
		}
	}
}
