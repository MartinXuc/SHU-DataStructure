...
//	辅助函数:
	...
	void PostRootOrder(ChildSiblingTreeNode<ElemType> *r, void (*Visit)(const ElemType &)) const;
		// 后根遍历以r为根的数
	void PreRootOrder(ChildSiblingTreeNode<ElemType>* r, void (*Visit)(const ElemType&), int& returnvalue) const;
...
template <class ElemType>
void ChildSiblingTree<ElemType>::PreRootOrder(ChildSiblingTreeNode<ElemType> *r, void (*Visit)(const ElemType &)) const
...
// 操作结果：将以troot为根的树复制成新的树, 求新树的根
{
	if (troot == NULL)
		...
		ChildSiblingTreeNode<ElemType> *subTreeRoot, *cur=NULL, *p;
...
// 操作结果：建立数据元素为items[],对应结点双亲为parents[],根结点位置为r,结点个数为n的树，并返回树的根
{
	if (r >= 0 && r < n){
		...
		ChildSiblingTreeNode<ElemType> *subTreeRoot, *cur=NULL;
