...
int main() {
    ...
    while(!fin.eof())
    {
        ...
        input_token_list.push_back(t);
    }
    //for (int i = 0; i < input_token_list.size(); i++)
    //    fout << input_token_list[i].first << '\t' << input_token_list[i].second << endl;
    try {
        if (input_token_list.empty()) {
